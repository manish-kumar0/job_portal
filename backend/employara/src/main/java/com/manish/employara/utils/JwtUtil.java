...
public class JwtUtil {
    ...
    @Value("${app.security.jwt.refresh-token.expiration}")
    private long refreshExpiration; // changed from int to long
    ...
    private String getToken(User user, long expirySeconds) {
        Date now = new Date();
        Date expiry = new Date(now.getTime() + expirySeconds * 1000L);
        return Jwts.builder()
                .subject(user.getEmail())
    ...
    public ResponseCookie generateCookie(User user, String name, long expirySeconds) {
        String token = getToken(user, expirySeconds);
